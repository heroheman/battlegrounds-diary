...
const getters = {
  ...
  resultById: state => resultId =>
    state.results.find(res => res.id === resultId),
  ...
  averageGainLose: state => {
    ...
    for (let i = 0; i < ranks; i++) {
      let items =
        state.results.filter(res => parseInt(res.placement) === i + 1) || {};
      let gains = 0;
  ...
  leaderboardHeroes: state => {
    ...
    for (const hero of state.heroes) {
      ...
      data.pointsAverage = parseFloat(precise(data.points / data.gamesPlayed));
